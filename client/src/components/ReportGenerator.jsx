...
const ReportGenerator = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch('/api/transit?limit=10000');
    ...
    const columns = [
        ...
        { key: 'DoorDescription', label: 'Location' },
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [filters, dateRange, customStart, customEnd]);
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {loading && (
                <div className="flex items-center justify-center h-64">
                    <div className="text-slate-400">Loading transit data...</div>
                </div>
            )}
                ...
                <>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                        <p className="text-sm text-slate-500 font-medium whitespace-nowrap">
                            Showing <span className="text-white">{Math.min(((currentPage - 1) * itemsPerPage) + 1, processedData.length)}</span> - <span className="text-white">{Math.min(currentPage * itemsPerPage, processedData.length)}</span> of <span className="text-white">{processedData.length}</span>
                        </p>
